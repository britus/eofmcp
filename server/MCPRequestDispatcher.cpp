...
MCPRequestDispatcher::MCPRequestDispatcher(MCPServer *pServer, QObject *pParent)
    : QObject(pParent)
    , m_pServer(pServer)
...
void MCPRequestDispatcher::initializeRoutes()
{
    ...
    m_pRouter->registerRoute("initialize", [this](const QSharedPointer<MCPContext> &pContext) { return m_pInitializeHandler->handleInitialize(pContext); });

    m_pRouter->registerRoute("notifications/initialized", [this](const QSharedPointer<MCPContext> &pContext) { return m_pInitializeHandler->handleInitialized(pContext); });

    m_pRouter->registerRoute("tools/list", [this](const QSharedPointer<MCPContext> &pContext) { return handleToolsList(pContext); });

    m_pRouter->registerRoute("tools/call", [this](const QSharedPointer<MCPContext> &pContext) { return handleToolsCall(pContext); });

    m_pRouter->registerRoute("resources/list", [this](const QSharedPointer<MCPContext> &pContext) { return handleListResources(pContext); });

    m_pRouter->registerRoute("resources/templates/list", [this](const QSharedPointer<MCPContext> &pContext) { return handleListResourceTemplates(pContext); });

    m_pRouter->registerRoute("resources/read", [this](const QSharedPointer<MCPContext> &pContext) { return handleReadResource(pContext); });

    m_pRouter->registerRoute("resources/subscribe", [this](const QSharedPointer<MCPContext> &pContext) { return m_pSubscriptionHandler->handleSubscribe(pContext); });

    m_pRouter->registerRoute("resources/unsubscribe", [this](const QSharedPointer<MCPContext> &pContext) { return m_pSubscriptionHandler->handleUnsubscribe(pContext); });

    m_pRouter->registerRoute("prompts/list", [this](const QSharedPointer<MCPContext> &pContext) { return handleListPrompts(pContext); });

    m_pRouter->registerRoute("prompts/get", [this](const QSharedPointer<MCPContext> &pContext) { return handleGetPrompt(pContext); });

    m_pRouter->registerRoute("notifications/subscribe", [this](const QSharedPointer<MCPContext> &pContext) { return m_pSubscriptionHandler->handleSubscribe(pContext); });

    m_pRouter->registerRoute("notifications/unsubscribe", [this](const QSharedPointer<MCPContext> &pContext) { return m_pSubscriptionHandler->handleUnsubscribe(pContext); });
}
...
{
    auto rc = QtConcurrent::run([this, pContext]() {
