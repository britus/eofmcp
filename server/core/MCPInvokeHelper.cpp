 ...
 * @brief MCP invocation helper implementation
 * @author zhangheng
 * @date 2025-01-01
...
{
    this->thread() == QThread::currentThread() ? (fun(), true) : (m_fun = fun, QMetaObject::invokeMethod(this, "invoke", Qt::BlockingQueuedConnection));
    deleteLater();
}
...
{
    bool bResult = false;
    // Directly use existing syncInvoke, capture return value through lambda
    syncInvoke(pTargetObj, [&bResult, fun]() { bResult = fun(); });
    return bResult;
}
