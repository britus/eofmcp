...
bool MCPResourceService::addFileResourceFromConfig(const MCPResourceConfig& resourceConfig)
{
    ...
    {
        MCP_CORE_LOG_WARNING() << "MCPResourceService: File resource configuration invalid (missing filePath):"
...
bool MCPResourceService::addWrapperResourceFromConfig(const MCPResourceConfig& resourceConfig, const QMap<QString, QObject*>& dictHandlers)
{
    ...
    {
        MCP_CORE_LOG_WARNING() << "MCPResourceService: Wrapper resource configuration invalid (missing handlerName):"
                               << resourceConfig.strUri;
        return false;
    }
    ...
    if (pHandler == nullptr)
    {
        MCP_CORE_LOG_WARNING() << "MCPResourceService: Resource Handler not found:" << resourceConfig.strHandlerName
                               << ", Resource URI:" << resourceConfig.strUri;
        return false;
    }
    ...
    if (pWrapper == nullptr)
    {
        MCP_CORE_LOG_WARNING() << "MCPResourceService: Failed to create resource wrapper:" << resourceConfig.strUri
...
bool MCPResourceService::addContentResourceFromConfig(const MCPResourceConfig& resourceConfig)
{
    ...
    {
        MCP_CORE_LOG_WARNING() << "MCPResourceService: Content resource configuration invalid (missing content):"
                               << resourceConfig.strUri;
        return false;
    }
...
bool MCPResourceService::subscribe(const QString& strUri, const QString& strSessionId)
{
    ...
    if (m_subscriptions.contains(strUri))
    {
        ...
        {
            MCP_CORE_LOG_DEBUG() << "MCPResourceService: Session" << strSessionId
                                << "already subscribed to URI:" << strUri;
    ...
    MCP_CORE_LOG_INFO() << "MCPResourceService: Session" << strSessionId
                       << "subscribed to URI:" << strUri;
    return true;
}
...
void MCPResourceService::unsubscribeAll(const QString& strSessionId)
{
    ...
    MCP_CORE_LOG_INFO() << "MCPResourceService: All subscriptions for session" << strSessionId
                       << "canceled, total of" << uris.size() << "subscriptions";
}
